...
def labeled_tube_plot(x, data_y, tube_y, labels, title="", xlabel="", ylabel="", axes=None, max_freq=None):
    x = np.asarray(x)
    data_y = np.asarray(data_y)
    ...
    axes.set_xlim(x.min(), max_freq)
    axes.set_ylim((data_y - tube_y).min(), (data_y + tube_y).max())
    axes.legend()


def spectral_plot(X_real: np.ndarray, X_fake: np.ndarray, fs, axes=None, title=None, max_freq=None):
    ...
    labeled_tube_plot(freqs,
                      ...
                      ["real", "sim"],
                     title, "Hz", "log(Amp)", axes, max_freq)
